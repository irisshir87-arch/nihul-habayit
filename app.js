...
const NAV_ITEMS = [
  ...
  { id: "wishes", label: "רצונות", icon: "♡" },
  { id: "trip", label: "טיול", icon: "🎒" },
];

const defaultState = {
  shopping: [
    { id: crypto.randomUUID(), name: "אבוקדו", quantity: 3, category: "פירות וירקות", unitPrice: 4.5, purchased: false, purchasedAt: null },
    { id: crypto.randomUUID(), name: "ביצים", quantity: 1, category: "מוצרי חלב", unitPrice: 14.9, purchased: false, purchasedAt: null },
    { id: crypto.randomUUID(), name: "חלב 3%", quantity: 2, category: "מוצרי חלב", unitPrice: 6.2, purchased: false, purchasedAt: null },
    { id: crypto.randomUUID(), name: "לחם מלא", quantity: 1, category: "מאפים", unitPrice: 11.9, purchased: false, purchasedAt: null },
    { id: crypto.randomUUID(), name: "גבינה לבנה", quantity: 1, category: "מוצרי חלב", unitPrice: 6.9, purchased: true, purchasedAt: new Date().toISOString() },
  ],
  memberEmails: { iris: "", tomer: "" },
  events: [
    { id: crypto.randomUUID(), title: "יום הולדת לסבתא רותי", date: "2026-07-25", startTime: "19:00", endTime: "22:00", location: "רמת גן", notes: "", participants: ["איריס", "תומר"], recurring: "none" },
    { id: crypto.randomUUID(), title: "בדיקת שיניים לאלון", date: "2026-07-27", startTime: "10:00", endTime: "10:30", location: "מרפאת ד״ר לוי", notes: "", participants: ["איריס"], recurring: "none" },
    { id: crypto.randomUUID(), title: "ערב זוגי", date: "2026-07-29", startTime: "20:30", endTime: "23:00", location: "תל אביב", notes: "", participants: ["איריס", "תומר"], recurring: "none" },
  ],
  tasks: [
    { id: crypto.randomUUID(), title: "להזמין טכנאי למזגן", assignee: "תומר", dueDate: "2026-07-22", priority: "גבוהה", category: "בית", notes: "המזגן בחדר הילדים", recurring: "none", completed: false, completedAt: null },
    { id: crypto.randomUUID(), title: "להחזיר ספרים לספרייה", assignee: "איריס", dueDate: "2026-07-23", priority: "בינונית", category: "ילדים", notes: "", recurring: "none", completed: false, completedAt: null },
    { id: crypto.randomUUID(), title: "לשלם חשבון מים", assignee: "איריס ותומר", dueDate: "2026-07-26", priority: "גבוהה", category: "כספים", notes: "", recurring: "none", completed: false, completedAt: null },
    { id: crypto.randomUUID(), title: "לקבוע תור לרופא", assignee: "איריס", dueDate: "2026-07-18", priority: "נמוכה", category: "בריאות", notes: "", recurring: "none", completed: true, completedAt: "2026-07-18T09:15:00" },
  ],
  wishes: [
...
function addLabel(screen) {
  return ({ shopping: "מוצר", events: "אירוע", tasks: "משימה", wishes: "רצון", trip: "פריט ציוד" })[screen] || "פריט";
}
...
function upcomingEvents() {
  ...
  return [...state.events]
    .filter((event) => new Date(`${event.date}T${event.startTime || "00:00"}`) >= today)
  ...
  return `
    <section class="grid stats-grid">
      <article class="card stat-card blue">
        <div class="card-title-row"><span class="stat-icon">✓</span><span class="stat-value">${activeTasks.length}</span></div>
        <div><div class="stat-label">משימות לביצוע</div><button class="link-button" data-nav="tasks">הצגת המשימות</button></div>
      </article>
      <article class="card stat-card green">
        <div class="card-title-row"><span class="stat-icon">📅</span><span class="stat-value">${events.length}</span></div>
        <div><div class="stat-label">אירועים קרובים</div><button class="link-button" data-nav="events">הצגת האירועים</button></div>
      </article>
    </section>

    <section class="grid home-grid">
      <article class="card">
        <div class="card-title-row"><h3 class="card-title">משימות קרובות</h3><button class="link-button" data-nav="tasks">הצג הכל</button></div>
        <div class="list">${activeTasks.slice().sort(sortTasks).slice(0, 4).map(taskItemHtml).join("") || emptyHtml("אין משימות לביצוע")}</div>
      </article>
      <article class="card">
        ...
        <div class="list">${events.slice(0, 4).map(eventItemHtml).join("") || emptyHtml("אין אירועים קרובים")}</div>
      </article>
      <article class="card">
        <div class="card-title-row"><h3 class="card-title">לוח חודשי</h3><span class="muted small">יולי 2026</span></div>
        ${calendarHtml(2026, 6)}
      </article>
    </section>

    <section class="card" style="margin-top:18px">
      <div class="card-title-row"><h3 class="card-title">פעולות מהירות</h3></div>
      <div class="grid quick-grid">
        <button class="quick-card" data-add="tasks"><span>✓</span>הוספת משימה</button>
        <button class="quick-card" data-add="events"><span>📅</span>הוספת אירוע</button>
        <button class="quick-card" data-add="shopping"><span>🛒</span>הוספת מוצר</button>
        <button class="quick-card" data-add="wishes"><span>♡</span>הוספת רצון</button>
      </div>
    </section>`;
}
...
function taskItemHtml(task) {
  return `<div class="list-item">
    ...
    <div class="list-meta">${formatDate(task.dueDate)} · ${escapeHtml(task.assignee)} · ${escapeHtml(task.category)}</div></div>
...
function eventItemHtml(event) {
  ...
  return `<div class="list-item">
    ...
    <div class="list-meta">${escapeHtml(event.startTime)}${event.endTime ? `–${escapeHtml(event.endTime)}` : ""} · ${escapeHtml(event.location || "ללא מיקום")}</div></div>
  </div>`;
}
...
function renderShopping() {
  ...
  const activeTotal = active.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
  const purchasedTotal = purchased.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
  return `<section class="card shopping-card">
    <div class="card-title-row">
      <div><h3 class="card-title">רשימת קניות</h3><p class="muted small">המחיר מוזן ידנית ליחידה · הסה״כ מחושב אוטומטית לפי הכמות</p></div>
      <button class="primary-button" data-add="shopping">＋ הוספת מוצר</button>
    </div>
    <div class="toolbar compact-toolbar"><input class="search-field" id="shopping-search" placeholder="חיפוש מוצר" /><select id="shopping-category"><option value="">כל הקטגוריות</option>${shoppingCategories().map((category) => `<option>${escapeHtml(category)}</option>`).join("")}</select></div>
    <div class="shopping-section-title"><h3 class="card-title">לקנות <span class="badge blue">${active.length}</span></h3><span class="price">${currency(activeTotal)}</span></div>
    <div class="shopping-groups">${shoppingGroupsHtml(active, false) || emptyHtml("רשימת הקניות ריקה")}</div>
    <div class="section-spacer"></div>
    <div class="shopping-section-title"><h3 class="card-title">נרכשו <span class="badge green">${purchased.length}</span></h3><span class="price">${currency(purchasedTotal)}</span></div>
    <div class="shopping-groups">${shoppingGroupsHtml(purchased, true) || emptyHtml("עדיין לא סומנו פריטים כנרכשו")}</div>
  </section>`;
}

function shoppingCategories() {
  return [...new Set(state.shopping.map((item) => item.category || "ללא קטגוריה"))].sort((a, b) => collator.compare(a, b));
}

function shoppingCategoryOptions(selected = "") {
  const defaults = ["פירות וירקות", "מוצרי חלב", "מאפים", "ניקיון", "מזווה", "תינוקות", "אחר"];
  return [...new Set([...defaults, ...shoppingCategories(), selected].filter(Boolean))]
...
function shoppingGroupsHtml(items, purchased) {
  ...
  return [...grouped.entries()]
    ...
    .map(([category, categoryItems]) => {
      ...
      const categoryTotal = sortedItems.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
      return `<section class="shopping-category-group" data-shopping-group data-category="${escapeHtml(category)}">
        <div class="shopping-category-header"><div><h4>${escapeHtml(category)}</h4><span class="badge ${purchased ? "green" : "blue"}">${sortedItems.length}</span></div><span class="small muted">${currency(categoryTotal)}</span></div>
...
function shoppingRowHtml(item) {
  ...
  return `<div class="shopping-row" data-shopping-row data-shopping-id="${item.id}" data-name="${escapeHtml(normalizeName(item.name))}" data-category="${escapeHtml(item.category || "ללא קטגוריה")}">
    ...
    <div class="shopping-product"><strong class="${item.purchased ? "strike" : ""}">${escapeHtml(item.name)}</strong>${item.purchased && item.purchasedAt ? `<span>נרכש ${formatDateTime(item.purchasedAt)}</span>` : ""}</div>
    ...
    <div class="shopping-price-pair" aria-label="מחירי המוצר">
      <span><small>ליחידה</small><strong>${currency(item.unitPrice)}</strong></span>
      <span class="shopping-total"><small>סה״כ</small><strong>${currency(totalPrice)}</strong></span>
    </div>
...
function shoppingInlineEditHtml(item) {
  return `<div class="shopping-row is-editing" data-shopping-row data-shopping-id="${item.id}" data-name="${escapeHtml(normalizeName(item.name))}" data-category="${escapeHtml(item.category || "ללא קטגוריה")}">
    ...
    <div class="shopping-edit-fields">
      ...
      <label><span>מחיר ליחידה</span><input data-inline-price type="number" min="0" step="0.01" value="${Number(item.unitPrice || 0)}" aria-label="מחיר ליחידה" /></label>
    </div>
    ...
    <div class="shopping-price-pair shopping-price-preview"><span class="shopping-total"><small>סה״כ</small><strong data-inline-total>${currency(item.quantity * item.unitPrice)}</strong></span></div>
...
function changeInlineShoppingQuantity(button, delta) {
  ...
  updateInlineShoppingTotal(row);
}

function updateInlineShoppingTotal(row) {
  const quantity = positiveInteger(row?.querySelector("[data-inline-quantity]")?.value);
  const price = Math.max(0, Number(row?.querySelector("[data-inline-price]")?.value) || 0);
  const total = row?.querySelector("[data-inline-total]");
  if (total) total.textContent = currency(quantity * price);
}
...
function saveInlineShoppingEdit(id) {
  ...
  item.unitPrice = Math.max(0, Number(row.querySelector("[data-inline-price]")?.value) || 0);
...
function filterShoppingRows() {
  ...
  const category = document.querySelector("#shopping-category")?.value || "";
  document.querySelectorAll("[data-shopping-group]").forEach((group) => {
    let visibleRows = 0;
    group.querySelectorAll("[data-shopping-row]").forEach((row) => {
      row.hidden = !(row.dataset.name.includes(query) && (!category || row.dataset.category === category));
...
function renderEvents() {
  ...
  return `<section class="card events-card">
    <div class="card-title-row"><div><h3 class="card-title">האירועים שלנו</h3><p class="muted small">רק איריס ותומר · כל אירוע מוצג בשורה אחת</p></div><button class="primary-button" data-add="events">＋ אירוע חדש</button></div>
    <div class="event-list-header"><span>תאריך</span><span>אירוע</span><span>שעה ומיקום</span><span>משתתפים</span><span></span></div>
...
function eventFullHtml(event) {
  ...
  return `<div class="compact-event-row">
    ...
    <div class="event-when-cell"><strong>${escapeHtml(event.startTime)}${event.endTime ? `–${escapeHtml(event.endTime)}` : ""}</strong><span>${escapeHtml(event.location || "ללא מיקום")}</span></div>
...
function renderTasks() {
  ...
  return `<section>
    <div class="card-title-row"><div><h3 class="card-title">משימות לביצוע</h3><p class="muted small">חלוקה לפי איריס, תומר ומשימות משותפות</p></div><button class="primary-button" data-add="tasks">＋ משימה חדשה</button></div>
    ...
    <div class="section-spacer"></div>
    <div class="card-title-row"><h3 class="card-title">משימות שבוצעו <span class="badge green">${completed.length}</span></h3></div>
...
function sortTasks(a, b) {
  return (a.dueDate || "9999").localeCompare(b.dueDate || "9999") || collator.compare(a.title, b.title);
}

function taskAssigneeSectionHtml(assignee, tasks, completed) {
  const heading = assignee === "איריס ותומר" ? "משימות משותפות" : `המשימות של ${assignee}`;
  const icon = assignee === "איריס" ? "א" : assignee === "תומר" ? "ת" : "יחד";
  return `<article class="card task-assignee-card ${completed ? "completed-card" : ""}">
    <div class="task-assignee-header"><span class="assignee-avatar">${icon}</span><div><h4>${heading}</h4><span class="muted small">${tasks.length} משימות</span></div></div>
    <div class="task-compact-list">${tasks.map((task) => taskCompactHtml(task, completed)).join("") || emptyHtml(completed ? "אין משימות שבוצעו" : "אין משימות לביצוע")}</div>
  </article>`;
}
...
function taskCompactHtml(task, completed) {
  return `<div class="task-compact-row">
    ...
    <div class="list-main"><div class="list-title ${completed ? "strike" : ""}">${escapeHtml(task.title)}</div><div class="list-meta">${completed ? `בוצע ${formatDateTime(task.completedAt)}` : `${formatDate(task.dueDate)} · ${escapeHtml(task.category)} · ${escapeHtml(task.priority)}`}${task.notes ? ` · ${escapeHtml(task.notes)}` : ""}</div></div>
...
function renderWishes() {
  return `<section>
    <div class="card-title-row"><div><h3 class="card-title">הרצונות שלנו</h3><p class="muted small">מחולק לבית, ילדים, תומר ואיריס</p></div><button class="primary-button" data-add="wishes">＋ רצון חדש</button></div>
...
function wishGroupHtml(group) {
  ...
  return `<section class="card wish-group-card"><div class="wish-group-header"><span class="assignee-avatar">${icon}</span><div><h3>${group}</h3><span class="muted small">${wishes.length} רעיונות</span></div></div><div class="wish-list">${wishes.map(wishHtml).join("") || emptyHtml("אין רעיונות עדיין")}</div></section>`;
}
...
function renderTrip() {
  ...
  return `<section>
    <div class="card trip-progress-card"><div class="card-title-row"><div><strong>${packed} מתוך ${total} פריטים ארוזים</strong><div class="muted small">רשימת ציוד משותפת לאיריס ולתומר · ביטול סימון מחזיר את הפריט לרשימה</div></div><strong class="progress-number">${progress}%</strong></div><div class="progress-track"><span style="width:${progress}%"></span></div></div>
    <div class="trip-category-grid">${TRIP_CATEGORIES.map(tripCategoryHtml).join("")}</div>
  </section>`;
}
...
function tripCategoryHtml(category) {
  ...
  return `<section class="card trip-category-card"><div class="trip-category-header"><span>${icon}</span><div><h3>${category}</h3><small>${items.length} פריטים</small></div></div><div class="trip-list">${items.map(tripItemHtml).join("") || emptyHtml("אין פריטים בקטגוריה")}</div></section>`;
}
...
function tripItemHtml(item) {
  return `<div class="trip-row">
    ...
    <div class="list-main"><div class="list-title ${item.packed ? "strike" : ""}">${escapeHtml(item.name)}</div>${item.packedAt ? `<div class="list-meta">ארוז ${formatDateTime(item.packedAt)}</div>` : ""}</div>
...
function attachScreenEvents() {
  ...
  document.querySelectorAll("[data-inline-quantity], [data-inline-price]").forEach((input) => input.addEventListener("input", () => updateInlineShoppingTotal(input.closest("[data-shopping-row]"))));
  ...
  document.querySelector("#shopping-category")?.addEventListener("change", filterShoppingRows);
...
function toggleTask(id) {
  ...
  saveState(task.completed ? "המשימה הועברה למשימות שבוצעו" : "המשימה הוחזרה לביצוע");
  render();
}
...
function dialogConfig(type, item) {
  ...
  const configs = {
    ...
    tasks: {
      eyebrow: "משימות", title: editing ? "עריכת משימה" : "משימה חדשה", html: taskFormHtml(item), submitLabel: editing ? "שמירת שינויים" : "שמירה",
      submit: (data) => submitTask(data, item?.id),
    },
    wishes: {
      eyebrow: "רצונות", title: editing ? "עריכת רצון" : "רצון חדש", html: wishFormHtml(item), submitLabel: editing ? "שמירת שינויים" : "שמירה",
...
function shoppingFormHtml(item = null) {
  return `<div class="form-stack">
    ...
    <label>מחיר ליחידה<input name="unitPrice" type="number" min="0" step="0.01" value="${Number(item?.unitPrice || 0)}" /></label>
...
function eventFormHtml(item = null) {
  return `<div class="form-stack">
    ...
    <div class="form-grid"><label>שעת התחלה<input name="startTime" type="time" required value="${escapeHtml(item?.startTime || "")}" /></label><label>שעת סיום<input name="endTime" type="time" value="${escapeHtml(item?.endTime || "")}" /></label></div>
...
function submitEvent(formData, id = null) {
  ...
  const values = {
    title: String(formData.get("title") || "").trim(), date: formData.get("date"), startTime: formData.get("startTime"), endTime: formData.get("endTime"),
...
function taskFormHtml(item = null) {
  return `<div class="form-stack">
    <label>שם המשימה<input name="title" required autofocus value="${escapeHtml(item?.title || "")}" /></label>
    <label>הערה<textarea name="notes">${escapeHtml(item?.notes || "")}</textarea></label>
    <div class="form-grid"><label>אחראי<select name="assignee">${TASK_ASSIGNEES.map((assignee) => `<option ${item?.assignee === assignee ? "selected" : ""}>${assignee}</option>`).join("")}</select></label><label>תאריך יעד<input name="dueDate" type="date" value="${escapeHtml(item?.dueDate || "")}" /></label></div>
    <div class="form-grid"><label>עדיפות<select name="priority">${["גבוהה", "בינונית", "נמוכה"].map((priority) => `<option ${item?.priority === priority || (!item && priority === "בינונית") ? "selected" : ""}>${priority}</option>`).join("")}</select></label><label>קטגוריה<select name="category">${["בית", "ילדים", "כספים", "בריאות", "עבודה", "אחר"].map((category) => `<option ${item?.category === category ? "selected" : ""}>${category}</option>`).join("")}</select></label></div>
    <label>חזרתיות<select name="recurring">${[["none", "ללא חזרה"], ["daily", "יומי"], ["weekly", "שבועי"], ["monthly", "חודשי"]].map(([value, label]) => `<option value="${value}" ${item?.recurring === value ? "selected" : ""}>${label}</option>`).join("")}</select></label>
  </div>`;
}

function submitTask(formData, id = null) {
  const values = {
    title: String(formData.get("title") || "").trim(), notes: String(formData.get("notes") || "").trim(), assignee: formData.get("assignee"), dueDate: formData.get("dueDate"),
  ...
  saveState(id ? "המשימה עודכנה" : "המשימה נוספה");
  dialog.close();
  render();
}
...
function wishFormHtml(item = null) {
  return `<div class="form-stack">
    <label>שם הרצון<input name="title" required autofocus value="${escapeHtml(item?.title || "")}" /></label>
...
function submitWish(formData, id = null) {
  ...
  saveState(id ? "הרצון עודכן" : "הרצון נוסף");
  dialog.close();
  render();
}
...
function downloadICS(eventId) {
  ...
  const ics = [
    ...
    `DTSTART:${start}`, `DTEND:${end}`, `SUMMARY:${escapeIcs(event.title)}`, `LOCATION:${escapeIcs(event.location || "")}`, `DESCRIPTION:${escapeIcs(event.notes || "")}`,
